package com.applex.matrimony.Pojo;

import com.google.gson.annotations.SerializedName;

import java.util.ArrayList;

public class ChildPojoProfile {

    @SerializedName("profile_id") String profile_id;
    @SerializedName("user_id") String user_id;
    @SerializedName("profile_for") String profile_for;
    @SerializedName("profile_name") String profile_name;
    @SerializedName("matrimony_id") String matrimony_id;
    @SerializedName("gender") String gender;
    @SerializedName("email") String email;
    @SerializedName("phone_countrycode") String phone_countrycode;
    @SerializedName("phone") String phone;
    @SerializedName("dob") String dob;
    @SerializedName("is_catholic") String is_catholic;
    @SerializedName("age") String age;
    @SerializedName("religion") String religion;
    @SerializedName("caste") String caste;
    @SerializedName("sub_caste") String sub_caste;
    @SerializedName("division") String division;
    @SerializedName("income") String income;
    @SerializedName("income_actual") String income_actual;
    @SerializedName("mother_language") String mother_language;
    @SerializedName("maritial_status") String maritial_status;
    @SerializedName("children_number") String children_number;
    @SerializedName("children_living") String children_living;
    @SerializedName("willing_intercast") String willing_intercast;
    @SerializedName("country") String country;
    @SerializedName("state") String state;
    @SerializedName("city") String city;
    @SerializedName("city_name_1") String city_name_1;
    @SerializedName("resident_status") String resident_status;
    @SerializedName("parish") String parish;
    @SerializedName("parish_village") String parish_village;
    @SerializedName("education") String education;
    @SerializedName("college") String college;
    @SerializedName("education_detail") String education_detail;
    @SerializedName("occupation") String occupation;
    @SerializedName("occupation_detail") String occupation_detail;
    @SerializedName("employed_in") String employed_in;
    @SerializedName("occupation_sector") String occupation_sector;
    @SerializedName("height") String height;
    @SerializedName("weight") String weight;
    @SerializedName("sports_id") String sports_id;
    @SerializedName("hobby_id") String hobby_id;
    @SerializedName("smoking") String smoking;
    @SerializedName("drinking") String drinking;
    @SerializedName("eating") String eating;
    @SerializedName("music_id") String music_id;
    @SerializedName("complexion") String complexion;
    @SerializedName("physical_status") String physical_status;
    @SerializedName("body_type") String body_type;
    @SerializedName("profile_photo") String profile_photo;
    @SerializedName("profile_photo_blured") String profile_photo_blured;
    @SerializedName("profile_preference") String profile_preference;
    @SerializedName("star") String star;
    @SerializedName("raasi") String raasi;
    @SerializedName("have_dosham") String have_dosham;
    @SerializedName("dosham_details") String dosham_details;
    @SerializedName("family_type") String family_type;
    @SerializedName("family_status") String family_status;
    @SerializedName("family_value") String family_value;
    @SerializedName("family_origin") String family_origin;
    @SerializedName("family_location") String family_location;
    @SerializedName("family_about") String family_about;
    @SerializedName("father_status") String father_status;
    @SerializedName("mother_status") String mother_status;
    @SerializedName("brothers") String brothers;
    @SerializedName("sisters") String sisters;
    @SerializedName("brothers_married") String brothers_married;
    @SerializedName("sisters_married") String sisters_married;
    @SerializedName("partner_preference") String partner_preference;
    @SerializedName("about_you") String about_you;
    @SerializedName("created_date") String created_date;
    @SerializedName("modified_date") String modified_date;
    @SerializedName("specialization") String specialization;
    @SerializedName("citizenship") String citizenship;
    @SerializedName("currency") String currency;
    @SerializedName("dnd") String dnd;
    @SerializedName("is_premium") String is_premium;
    @SerializedName("is_phone_verified") String is_phone_verified;
    @SerializedName("profile_approval") String profile_approval;
    @SerializedName("profile_status") String profile_status;
    @SerializedName("email_unique_id") String email_unique_id;
    @SerializedName("profile_highlight") String profile_highlight;
    @SerializedName("gothra") String gothra;
    @SerializedName("birth_time") String birth_time;
    @SerializedName("birth_country") String birth_country;
    @SerializedName("birth_state") String birth_state;
    @SerializedName("birth_city") String birth_city;
    @SerializedName("chart") String chart;
    @SerializedName("shortlisted") String shortlisted;
    @SerializedName("interested") String interested;
    @SerializedName("Gallery") ArrayList<String> Gallery;



    public String getFamily_location ()
    {
        return family_location;
    }

    public void setFamily_location (String family_location)
    {
        this.family_location = family_location;
    }

    public String getEducation_detail ()
    {
        return education_detail;
    }

    public void setEducation_detail (String education_detail)
    {
        this.education_detail = education_detail;
    }

    public String getBirth_country ()
    {
        return birth_country;
    }

    public void setBirth_country (String birth_country)
    {
        this.birth_country = birth_country;
    }

    public String getSisters_married ()
    {
        return sisters_married;
    }

    public void setSisters_married (String sisters_married)
    {
        this.sisters_married = sisters_married;
    }

    public String getPhone ()
    {
        return phone;
    }

    public void setPhone (String phone)
    {
        this.phone = phone;
    }

    public String getEmployed_in ()
    {
        return employed_in;
    }

    public void setEmployed_in (String employed_in)
    {
        this.employed_in = employed_in;
    }

    public String getFather_status ()
    {
        return father_status;
    }

    public void setFather_status (String father_status)
    {
        this.father_status = father_status;
    }

    public String getDrinking ()
    {
        return drinking;
    }

    public void setDrinking (String drinking)
    {
        this.drinking = drinking;
    }

    public String getProfile_photo ()
    {
        return profile_photo;
    }

    public void setProfile_photo (String profile_photo)
    {
        this.profile_photo = profile_photo;
    }

    public String getCollege ()
    {
        return college;
    }

    public void setCollege (String college)
    {
        this.college = college;
    }

    public String getCity ()
    {
        return city;
    }

    public void setCity (String city)
    {
        this.city = city;
    }

    public String getReligion ()
    {
        return religion;
    }

    public void setReligion (String religion)
    {
        this.religion = religion;
    }

    public String getSisters ()
    {
        return sisters;
    }

    public void setSisters (String sisters)
    {
        this.sisters = sisters;
    }

    public String getFamily_about ()
    {
        return family_about;
    }

    public void setFamily_about (String family_about)
    {
        this.family_about = family_about;
    }

    public String getHeight ()
    {
        return height;
    }

    public void setHeight (String height)
    {
        this.height = height;
    }

    public String getPartner_preference ()
    {
        return partner_preference;
    }

    public void setPartner_preference (String partner_preference)
    {
        this.partner_preference = partner_preference;
    }

    public String getIncome ()
    {
        return income;
    }

    public void setIncome (String income)
    {
        this.income = income;
    }

    public String getAge ()
    {
        return age;
    }

    public void setAge (String age)
    {
        this.age = age;
    }

    public String getParish ()
    {
        return parish;
    }

    public void setParish (String parish)
    {
        this.parish = parish;
    }

    public String getFamily_value ()
    {
        return family_value;
    }

    public void setFamily_value (String family_value)
    {
        this.family_value = family_value;
    }

    public String getGender ()
    {
        return gender;
    }

    public void setGender (String gender)
    {
        this.gender = gender;
    }

    public String getHobby_id ()
    {
        return hobby_id;
    }

    public void setHobby_id (String hobby_id)
    {
        this.hobby_id = hobby_id;
    }

    public String getHave_dosham ()
    {
        return have_dosham;
    }

    public void setHave_dosham (String have_dosham)
    {
        this.have_dosham = have_dosham;
    }

    public String getProfile_for ()
    {
        return profile_for;
    }

    public void setProfile_for (String profile_for)
    {
        this.profile_for = profile_for;
    }

    public String getGothra ()
    {
        return gothra;
    }

    public void setGothra (String gothra)
    {
        this.gothra = gothra;
    }

    public String getBrothers ()
    {
        return brothers;
    }

    public ArrayList<String> getGallery() {
        return Gallery;
    }

    public void setGallery(ArrayList<String> gallery) {
        Gallery = gallery;
    }

    public void setBrothers (String brothers)
    {
        this.brothers = brothers;
    }

    public String getCreated_date ()
    {
        return created_date;
    }

    public void setCreated_date (String created_date)
    {
        this.created_date = created_date;
    }

    public String getSports_id ()
    {
        return sports_id;
    }

    public void setSports_id (String sports_id)
    {
        this.sports_id = sports_id;
    }

    public String getOccupation ()
    {
        return occupation;
    }

    public void setOccupation (String occupation)
    {
        this.occupation = occupation;
    }

    public String getCountry ()
    {
        return country;
    }

    public void setCountry (String country)
    {
        this.country = country;
    }

    public String getWilling_intercast ()
    {
        return willing_intercast;
    }

    public void setWilling_intercast (String willing_intercast)
    {
        this.willing_intercast = willing_intercast;
    }

    public String getProfile_highlight ()
    {
        return profile_highlight;
    }

    public void setProfile_highlight (String profile_highlight)
    {
        this.profile_highlight = profile_highlight;
    }

    public String getEmail ()
    {
        return email;
    }

    public void setEmail (String email)
    {
        this.email = email;
    }

    public String getProfile_status ()
    {
        return profile_status;
    }

    public void setProfile_status (String profile_status)
    {
        this.profile_status = profile_status;
    }

    public String getOccupation_sector ()
    {
        return occupation_sector;
    }

    public void setOccupation_sector (String occupation_sector)
    {
        this.occupation_sector = occupation_sector;
    }

    public String getIs_premium ()
    {
        return is_premium;
    }

    public void setIs_premium (String is_premium)
    {
        this.is_premium = is_premium;
    }

    public String getFamily_status ()
    {
        return family_status;
    }

    public void setFamily_status (String family_status)
    {
        this.family_status = family_status;
    }

    public String getProfile_photo_blured ()
    {
        return profile_photo_blured;
    }

    public void setProfile_photo_blured (String profile_photo_blured)
    {
        this.profile_photo_blured = profile_photo_blured;
    }

    public String getMaritial_status ()
    {
        return maritial_status;
    }

    public void setMaritial_status (String maritial_status)
    {
        this.maritial_status = maritial_status;
    }

    public String getMatrimony_id ()
    {
        return matrimony_id;
    }

    public void setMatrimony_id (String matrimony_id)
    {
        this.matrimony_id = matrimony_id;
    }

    public String getParish_village ()
    {
        return parish_village;
    }

    public void setParish_village (String parish_village)
    {
        this.parish_village = parish_village;
    }

    public String getBirth_state ()
    {
        return birth_state;
    }

    public void setBirth_state (String birth_state)
    {
        this.birth_state = birth_state;
    }

    public String getProfile_id ()
    {
        return profile_id;
    }

    public void setProfile_id (String profile_id)
    {
        this.profile_id = profile_id;
    }

    public String getWeight ()
    {
        return weight;
    }

    public void setWeight (String weight)
    {
        this.weight = weight;
    }

    public String getModified_date ()
    {
        return modified_date;
    }

    public void setModified_date (String modified_date)
    {
        this.modified_date = modified_date;
    }

    public String getIncome_actual ()
    {
        return income_actual;
    }

    public void setIncome_actual (String income_actual)
    {
        this.income_actual = income_actual;
    }

    public String getPhone_countrycode ()
    {
        return phone_countrycode;
    }

    public void setPhone_countrycode (String phone_countrycode)
    {
        this.phone_countrycode = phone_countrycode;
    }

    public String getState ()
    {
        return state;
    }

    public void setState (String state)
    {
        this.state = state;
    }

    public String getCaste ()
    {
        return caste;
    }

    public void setCaste (String caste)
    {
        this.caste = caste;
    }

    public String getCitizenship ()
    {
        return citizenship;
    }

    public void setCitizenship (String citizenship)
    {
        this.citizenship = citizenship;
    }

    public String getIs_catholic ()
    {
        return is_catholic;
    }

    public void setIs_catholic (String is_catholic)
    {
        this.is_catholic = is_catholic;
    }

    public String getEducation ()
    {
        return education;
    }

    public void setEducation (String education)
    {
        this.education = education;
    }

    public String getInterested ()
    {
        return interested;
    }

    public void setInterested (String interested)
    {
        this.interested = interested;
    }

    public String getChildren_number ()
    {
        return children_number;
    }

    public void setChildren_number (String children_number)
    {
        this.children_number = children_number;
    }

    public String getCurrency ()
    {
        return currency;
    }

    public void setCurrency (String currency)
    {
        this.currency = currency;
    }

    public String getDivision ()
    {
        return division;
    }

    public void setDivision (String division)
    {
        this.division = division;
    }

    public String getMusic_id ()
    {
        return music_id;
    }

    public void setMusic_id (String music_id)
    {
        this.music_id = music_id;
    }

    public String getSub_caste ()
    {
        return sub_caste;
    }

    public void setSub_caste (String sub_caste)
    {
        this.sub_caste = sub_caste;
    }

    public String getCity_name_1 ()
    {
        return city_name_1;
    }

    public void setCity_name_1 (String city_name_1)
    {
        this.city_name_1 = city_name_1;
    }

    public String getSmoking ()
    {
        return smoking;
    }

    public void setSmoking (String smoking)
    {
        this.smoking = smoking;
    }

    public String getDnd ()
    {
        return dnd;
    }

    public void setDnd (String dnd)
    {
        this.dnd = dnd;
    }

    public String getBirth_city ()
    {
        return birth_city;
    }

    public void setBirth_city (String birth_city)
    {
        this.birth_city = birth_city;
    }

    public String getUser_id ()
    {
        return user_id;
    }

    public void setUser_id (String user_id)
    {
        this.user_id = user_id;
    }

    public String getFamily_type ()
    {
        return family_type;
    }

    public void setFamily_type (String family_type)
    {
        this.family_type = family_type;
    }

    public String getSpecialization ()
    {
        return specialization;
    }

    public void setSpecialization (String specialization)
    {
        this.specialization = specialization;
    }

    public String getBirth_time ()
    {
        return birth_time;
    }

    public void setBirth_time (String birth_time)
    {
        this.birth_time = birth_time;
    }

    public String getEmail_unique_id ()
    {
        return email_unique_id;
    }

    public void setEmail_unique_id (String email_unique_id)
    {
        this.email_unique_id = email_unique_id;
    }

    public String getBrothers_married ()
    {
        return brothers_married;
    }

    public void setBrothers_married (String brothers_married)
    {
        this.brothers_married = brothers_married;
    }

    public String getResident_status ()
    {
        return resident_status;
    }

    public void setResident_status (String resident_status)
    {
        this.resident_status = resident_status;
    }

    public String getDosham_details ()
    {
        return dosham_details;
    }

    public void setDosham_details (String dosham_details)
    {
        this.dosham_details = dosham_details;
    }

    public String getMother_language ()
    {
        return mother_language;
    }

    public void setMother_language (String mother_language)
    {
        this.mother_language = mother_language;
    }

    public String getPhysical_status ()
    {
        return physical_status;
    }

    public void setPhysical_status (String physical_status)
    {
        this.physical_status = physical_status;
    }

    public String getBody_type ()
    {
        return body_type;
    }

    public void setBody_type (String body_type)
    {
        this.body_type = body_type;
    }

    public String getProfile_name ()
    {
        return profile_name;
    }

    public void setProfile_name (String profile_name)
    {
        this.profile_name = profile_name;
    }

    public String getChildren_living ()
    {
        return children_living;
    }

    public void setChildren_living (String children_living)
    {
        this.children_living = children_living;
    }

    public String getAbout_you ()
    {
        return about_you;
    }

    public void setAbout_you (String about_you)
    {
        this.about_you = about_you;
    }

    public String getStar ()
    {
        return star;
    }

    public void setStar (String star)
    {
        this.star = star;
    }

    public String getProfile_approval ()
    {
        return profile_approval;
    }

    public void setProfile_approval (String profile_approval)
    {
        this.profile_approval = profile_approval;
    }

    public String getComplexion ()
    {
        return complexion;
    }

    public void setComplexion (String complexion)
    {
        this.complexion = complexion;
    }

    public String getOccupation_detail ()
    {
        return occupation_detail;
    }

    public void setOccupation_detail (String occupation_detail)
    {
        this.occupation_detail = occupation_detail;
    }

    public String getMother_status ()
    {
        return mother_status;
    }

    public void setMother_status (String mother_status)
    {
        this.mother_status = mother_status;
    }

    public String getDob ()
    {
        return dob;
    }

    public void setDob (String dob)
    {
        this.dob = dob;
    }

    public String getChart ()
    {
        return chart;
    }

    public void setChart (String chart)
    {
        this.chart = chart;
    }

    public String getEating ()
    {
        return eating;
    }

    public void setEating (String eating)
    {
        this.eating = eating;
    }

    public String getIs_phone_verified ()
    {
        return is_phone_verified;
    }

    public void setIs_phone_verified (String is_phone_verified)
    {
        this.is_phone_verified = is_phone_verified;
    }

    public String getFamily_origin ()
    {
        return family_origin;
    }

    public void setFamily_origin (String family_origin)
    {
        this.family_origin = family_origin;
    }

    public String getShortlisted ()
    {
        return shortlisted;
    }

    public void setShortlisted (String shortlisted)
    {
        this.shortlisted = shortlisted;
    }

    public String getRaasi ()
    {
        return raasi;
    }

    public void setRaasi (String raasi)
    {
        this.raasi = raasi;
    }

    public String getProfile_preference ()
    {
        return profile_preference;
    }

    public void setProfile_preference (String profile_preference)
    {
        this.profile_preference = profile_preference;
    }}
